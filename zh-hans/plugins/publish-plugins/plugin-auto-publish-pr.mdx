...
当插件贡献者需要严肃更新 **已被其他用户使用的天智插件** 时，流程通常非常繁琐：贡献者需要先修改插件源码并更新版本号，将更改推送到插件源码仓库，并在 fork 的 dify-plugin 仓库中创建新分支。随后，贡献者需要手动打包插件并上传打包文件，再创建 PR 合并到原始 dify-plugin 仓库。这个过程必须在每次插件代码变更时重复，非常耗时低效。
...
[  ](https://github.com/langgenius/dify-docs/edit/main/zh-hans/plugins/publish-plugins/plugin-auto-publish-pr.mdx) | [  ](https://github.com/langgenius/dify-docs/issues/new?template=docs.yml)
