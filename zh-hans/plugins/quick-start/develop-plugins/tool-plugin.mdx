...
工具指的是能够被 Chatflow / Workflow / Agent 类型应用所调用的第三方服务，提供完整的 API 实现能力，用于增强天智应用的能力。例如为应用添加在线搜索、图片生成等额外功能。
...
* `parameters` 参数列表
  ...
  * `options` 参数选项
    * 在 `llm` 模式下，天智 会将所有选项传递给 LLM，LLM 可以根据这些选项进行推理。
...
插件开发完成后，接下来需要测试插件是否可以正常运行。天智 提供便捷地远程调试方式，帮助你快速在测试环境中验证插件功能。
...
如果想要将插件发布至天智Marketplace，请确保你的插件遵循了[插件发布规范](https://docs.dify.ai/zh-hans/plugins/publish-plugins/publish-to-dify-marketplace)。审核通过后，代码将合并至主分支并自动上线至 [天智 Marketplace](https://marketplace.dify.ai/)。
...
* [反向调用天智能力](../../schema-definition/reverse-invocation-of-the-dify-service/)
...
[  ](https://github.com/langgenius/dify-docs/edit/main/zh-hans/plugins/quick-start/develop-plugins/tool-plugin.mdx) | [  ](https://github.com/langgenius/dify-docs/issues/new?template=docs.yml)
