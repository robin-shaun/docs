...
Tools:
    Enabled: [✔]  You can invoke tools inside天智if it's enabled # 默认开启
Models:
    Enabled: [✔]  You can invoke models inside天智if it's enabled # 默认开启
    LLM: [✔]  You can invoke LLM models inside天智if it's enabled # 默认开启
  → Text Embedding: [✘]  You can invoke text embedding models inside天智if it's enabled
    Rerank: [✘]  You can invoke rerank models inside天智if it's enabled
    TTS: [✘]  You can invoke TTS models inside天智if it's enabled
    Speech2Text: [✘]  You can invoke speech2text models inside天智if it's enabled
    Moderation: [✘]  You can invoke moderation models inside天智if it's enabled
Apps:
...
配置插件的声明文件与功能代码后，在插件的目录内运行 `python -m main` 命令重启插件。接下来需测试插件是否可以正常运行。天智 提供远程调试方式，前往[“插件管理”](https://cloud.dify.ai/plugins)获取调试 Key 和远程服务器地址。
...
现在可以将它上传至 [天智 Plugins 代码仓库](https://github.com/langgenius/dify-plugins)来发布你的插件了！上传前，请确保插件已遵循[插件发布规范](https://docs.dify.ai/zh-hans/plugins/publish-plugins/publish-to-dify-marketplace)。审核通过后，代码将合并至主分支并自动上线至 [天智 Marketplace](https://marketplace.dify.ai/)。
...
[  ](https://github.com/langgenius/dify-docs/edit/main/zh-hans/plugins/quick-start/develop-plugins/agent-strategy-plugin.mdx) | [  ](https://github.com/langgenius/dify-docs/issues/new?template=docs.yml)
