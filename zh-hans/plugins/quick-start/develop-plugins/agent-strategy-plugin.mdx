...
Tools:
    Enabled: [✔]  You can invoke tools inside 天智 if it's enabled # 默认开启
Models:
    Enabled: [✔]  You can invoke models inside 天智 if it's enabled # 默认开启
    LLM: [✔]  You can invoke LLM models inside 天智 if it's enabled # 默认开启
  → Text Embedding: [✘]  You can invoke text embedding models inside 天智 if it's enabled
    Rerank: [✘]  You can invoke rerank models inside 天智 if it's enabled
    TTS: [✘]  You can invoke TTS models inside 天智 if it's enabled
    Speech2Text: [✘]  You can invoke speech2text models inside 天智 if it's enabled
    Moderation: [✘]  You can invoke moderation models inside 天智 if it's enabled
Apps:
