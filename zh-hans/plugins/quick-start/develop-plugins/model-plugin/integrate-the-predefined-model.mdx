...
当模型调用异常时需要映射到 Runtime 指定的 `InvokeError` 类型，方便天智针对不同错误做不同后续处理。
...
接下来需测试插件是否可以正常运行。天智 提供远程调试方式，前往“插件管理”页获取调试 Key 和远程服务器地址。
...
现在可以将它上传至 [天智 Plugins 代码仓库](https://github.com/langgenius/dify-plugins) 来发布你的插件了！上传前，请确保你的插件遵循了[插件发布规范](https://docs.dify.ai/zh-hans/plugins/publish-plugins/publish-to-dify-marketplace)。审核通过后，代码将合并至主分支并自动上线至 [天智 Marketplace](https://marketplace.dify.ai/)。
...
[  ](https://github.com/langgenius/dify-docs/edit/main/zh-hans/plugins/quick-start/develop-plugins/model-plugin/integrate-the-predefined-model.mdx) | [  ](https://github.com/langgenius/dify-docs/issues/new?template=docs.yml)
