...
> * **Slack Bot 插件**指的是 天智 Marketplace上的一款插件，用于连接 天智 应用与 Slack 平台。本文将主要围绕该插件开发展开。

**原理简介：**

1.  **向 Slack Bot 发送消息**

    当用户在 Slack 中向 Bot 机器人发出一条消息的时候，Slack Bot 会发出一个 Webhook 请求到 天智 平台。
2.  **消息转发至 Slack Bot 插件**

    用户在与 Slack bot 对话时，需要将消息转发至 天智 应用。好比邮件系统需要一位收件人的邮箱，此时可以通过 Slack 的 API 配置一个 Slack Webhook 的地址，并将其填入至 Slack Bot 插件并建立连接。
3.  **插件在接受到消息后，返回至某个 天智 应用**

    Slack Bot 插件将处理 Slack 请求，发送至 天智 中的应用。由 LLM 分析用户输入的内容并给出回应。
4.  **天智 应用回应后，将消息返回至 Slack Bot 并回答用户**

    Slack Bot 获取 天智 应用的回复后，通过插件将消息原路返回至 Slack Bot，使得用户能够在使用 Slack 时直接与 天智 应用互动

### 前置准备

* 天智 插件脚手架工具，详细说明请参考[安装插件开发工具](../initialize-development-tools)。
...
现在开始实际的插件编码工作。在开始之前，请确保你已经阅读过[快速开始：开发 Extension 插件](/zh-hans/plugins/quick-start/develop-plugins/extension-plugin)，或已动手开发过一次 天智 插件。
...
如需了解更多关于插件反向调用 天智 平台能力，请参考[反向调用：App](../../../schema-definition/reverse-invocation-of-the-dify-service/app)。

![Plugins permission](https://assets-docs.dify.ai/2024/12/d89a6282c5584fc43a9cadeddf09c0de.png)

#### 1. 编辑配置表单

在这个插件中，需要指定使用哪个 天智 的 App 进行回复，并且在回复的时候需要使用到 Slack 的 App token，因此需要在插件表单中加上这两个字段。
...
为了便于测试，插件功能目前仅能重复用户输入的内容，暂不调用 天智 app。

### 2. 调试插件

前往 天智 平台，获取 天智 插件远程调试的连接地址和密钥。
...
在 天智 的插件管理页中找到自动安装的测试插件，新建一个 Endpoint，填写名称、Bot token、选择需要连接的 app。
...
代码使用了 `self.session.app.chat.invoke` 调用 天智 平台内的 App，并传递了 `app_id` 和 `query` 等信息，最后将 response 的内容返回至 Slack Bot。运行 `python -m main` 命令重启插件进行调试，确认 Slack Bot 是否能够正确输出 天智 App 的答复消息。
...
如果你想要查看完整 天智 插件的项目代码，请前往 [GitHub 代码仓库](https://github.com/langgenius/dify-plugins)。除此之外，你可以看到其它插件的完整代码与具体细节。
...
* [反向调用 天智 能力](/zh-hans/plugins/schema-definition/reverse-invocation-of-the-dify-service/)
