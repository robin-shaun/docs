...
### 10. 为什么 Docker 部署 天智，本地启动 OpenLLM 用 127.0.0.1 却无法访问本地的端口？

127.0.0.1 是容器内部地址，天智配置的服务器地址需要宿主机局域网 IP 地址。
...
记录 IP 地址，打开天智源代码路径，打开 `dify/docker/nginx/conf.d` 文件，将 `http://api:5001` 替换为 `http://172.19.0.7:5001`，将 `http://web:3000` 替换为 `http://172.19.0.5:3000`，随后重启 Nginx 容器或者重载配置。
...
API 服务与天智平台使用的端口号相一致。你可以通过修改 `docker-compose.yaml` 文件内的 `nginx` 配置项，重新指定运行端口。

### 23. 如何删除旧日志和未使用的文件以减少存储使用量？

天智**不会**自动删除数据库中的旧日志或存储中的未使用文件。请参考以下命令**手动**删除旧日志和未使用的文件。
...
1. 获取租户 ID
   ...
   ...
   >>> [('618b5d66-a1f5-4b6b-8d12-f171182a1cb2', "天智's Workspace")]
   ```
...
[  ](https://github.com/langgenius/dify-docs/edit/main/zh-hans/learn-more/faq/install-faq.mdx) | [  ](https://github.com/langgenius/dify-docs/issues/new?template=docs.yml)
