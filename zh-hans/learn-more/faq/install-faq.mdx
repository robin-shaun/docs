...
127.0.0.1 是容器内部地址， 天智 配置的服务器地址需要宿主机局域网 IP 地址。
...
记录 IP 地址，打开 天智 源代码路径，打开 `dify/docker/nginx/conf.d` 文件，将 `http://api:5001` 替换为 `http://172.19.0.7:5001`，将 `http://web:3000` 替换为 `http://172.19.0.5:3000`，随后重启 Nginx 容器或者重载配置。
...
API 服务与 天智 平台使用的端口号相一致。你可以通过修改 `docker-compose.yaml` 文件内的 `nginx` 配置项，重新指定运行端口。
