...
我们首先将在 天智 上完成三步操作，搭建出一个基于内部数据的 AI Sales Avatar “Vendy 本体”。  

---

### 🧩 Step 1：注册 天智 账号并配置模型 API

👉 打开 [天智 官网](https://dify.ai)，点击右上角 “Get Started”，开始注册流程。

登录 天智 后，在左侧菜单点击 **「Settings > Model Provider」（模型提供商）**。在页面中找到 **OpenAI** 模块，点击 **Install** 进行安装。
...
现在可以开始在 天智 上构建 AI Sales Avatar 的知识本体了。

---

### 🧩 Step 2：上传 AI Sales Avatar 所需的内部资料（知识文档）

在 天智 控制台顶部菜单，点击 **「Knowledge」**，  
...
下载好文档后，回到 天智 上传页面，选择 **「Import from file」**，上传该文件。  
...
✅ 若你看到以下页面，表示你的文档已经成功上传并被 天智 系统解析完成。
...
进入 天智 控制台，点击顶部菜单栏的 **「Studio」**，再点击右上角的 **「Create」** 创建新 App。
...
在将 天智 的智能体、Trulience 的数字人接入 TEN 平台之前，我们需要提前准备好语音通信和语音识别/合成所需的账号与密钥，主要包括以下服务：
...
这样即可将 TEN Agent 的大语言模型（LLM）模块，接入 天智 提供的智能体接口。
...
- 在 **API Key** 字段中，输入你在 天智 平台上创建的 API Key  
...
- 🤖 你可以根据她的回答，验证她是否成功读取并理解了 天智 知识库的内容。
