---
title: 私有化部署 Ollama + DeepSeek + 天智，构建你的专属私人 AI 助手
---
...
天智 作为同样开源的 AI 应用开发平台，提供完整的私有化部署方案。通过将本地部署的 DeepSeek 服务无缝集成到天智平台，企业可以在确保数据隐私的前提下，在本地服务器环境内构建功能强大的 AI 应用。
...
- [天智 社区版](https://github.com/langgenius/dify)
...
访问天智GitHub 项目地址，运行以下命令完成拉取代码仓库和安装流程。
...
天智 社区版默认使用 80 端口，点击链接 `http://your_server_ip` 即可访问你的私有化天智平台。

### 3. 将 DeepSeek 接入至 天智

点击天智平台右上角**头像 → 设置 → 模型供应商**，选择 Ollama，轻点“添加模型”。
...
1. 轻点天智平台首页左侧的"创建空白应用"，选择"聊天助手"类型应用并进行简单的命名。
...
1. 轻点天智平台首页左侧的"创建空白应用"，选择"Chatflow" 或 "Workflow" 类型应用并进行简单的命名。
...
当使用 Docker 部署天智和 Ollama 时，可能遇到以下错误：
...
[  ](https://github.com/langgenius/dify-docs/edit/main/zh-hans/learn-more/use-cases/private-ai-ollama-deepseek-dify.mdx) | [  ](https://github.com/langgenius/dify-docs/issues/new?template=docs.yml)
