...
本文将简要介绍如何通过外部知识库 API 将 天智 平台与 AWS Bedrock 知识库相连接，使得 天智 平台内的 AI 应用能够直接获取存储在 AWS Bedrock 知识库中的内容，扩展新的信息来源渠道。
...
天智 平台尚不能直接连接 AWS Bedrock Knowledge Base，需要开发团队参考 天智 关于外部知识库连接的 [API 定义](../../guides/knowledge-base/external-knowledge-api-documentation)，手动创建后端 API 服务，建立与 AWS Bedrock 的连接。具体架构示意图请参考：
...
登录 AWS Bedrock Knowledge 后台，获取已创建 Knowledge Base 的 ID。此参数将会在[后续步骤](how-to-connect-aws-bedrock.md#id-5.-lian-jie-wai-bu-zhi-shi-ku)用于与 天智 平台的连接。
...
前往 天智 平台中的 **“知识库”** 页，点击右上角的 **“外部知识库 API”**，轻点 **“添加外部知识库 API”**。

按照页面提示，依次填写以下内容：

* 知识库的名称，允许自定义名称，用于区分 天智 平台内所连接的不同外部知识 API；
