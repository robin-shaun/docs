---
title: AI 机器人开发：融云 IM 与 天智 集成实战
---
...
## 1. 概述
[融云 IM PaaS](https://www.rongcloud.cn/) 通过与 天智 平台的深度无缝集成，打通了 C 端用户与大语言模型（LLM）之间的端到端对话链路，支持流式响应和多轮上下文，广泛应用于智能问答、内容创作、情感陪聊等场景，助力企业构建高效、智能的用户交互体验。
...
本指南将介绍如何基于融云 IM 平台集成 AI 机器人，并接入 天智 平台大模型服务。
...
2. **用户终端**
  * 发送用户消息，接收机器人响应。
3. **IM Server**
* **消息转发**：根据回调类型（如 dify\_chat, dify\_completion）转发至 天智 对应接口，支持流式/非流式模式。
* **响应处理与会话管理**：
  * 流式：实时推送 天智 增量内容。
  * 非流式：整合完整文本后发送。
...
| **dify\_chat**       | 接入 天智 平台的“聊天助手”模式，用于构建多轮对话机器人。   |
| **dify\_completion** | 接入 天智 平台的“文本生成”模式，用于生成式内容应用场景。   |
| **dify\_chatflow**   | 接入 天智 平台的“Chatflow”模式，用于流程化对话构建。 |

## 3.3 接入步骤

1. **创建 天智 应用**：在 天智 中创建应用，并获取对应的 API 密钥（API Key）。
2. **创建真实用户**：在系统中创建一个用于与机器人交互的真实用户账号。
3. **配置机器人与回调地址**：创建机器人，并配置消息回调地址。当用户向机器人发送消息时，消息将回调至您的系统或 天智 平台。
4. **验证接入效果**：使用第二步创建的用户账号向机器人发送一条单聊消息，确认用户终端是否能收到来自 天智 的对话响应。
5. **验证连续对话效果**：使用同一用户持续向同一机器人发送消息，验证对话上下文是否正确延续。融云将自动维持与 天智 的上下文会话，实现连续对话体验。
...
使用第二步创建的用户账号向机器人发送一条单聊消息，确认用户终端是否能收到来自 天智 的对话响应。
...
使用同一用户持续向同一机器人发送消息，验证对话上下文是否正确延续。融云将自动维持与 天智 的上下文会话，实现连续对话体验。

<img src="https://docs.rongcloud.cn/platform-chat-api/assets/bot/send-bot-message3.png" alt="图示" width="50%" />


> 天智 日志显示共记录了 4 条消息，验证了多轮对话过程中上下文得以正确延续，说明聊天会话在同一上下文中持续进行。
...
## 5.2 为不同场景配置独立机器人与 天智 应用

建议根据具体业务场景（如知识问答、翻译、工具使用、陪聊等）创建不同类型的机器人，并为每个机器人配置独立的 天智 应用与专属提示词，以实现更精准的能力定位与更优质的用户体验。
...
感谢您通过本指南深入了解了融云 IM PaaS 平台与 天智 大模型服务的集成过程。这一结合不仅简化了开发者在智能机器人开发中的技术门槛，还通过高效的回调机制和多轮对话支持，赋予了应用更强的交互能力与商业价值。

无论是构建知识问答助手、内容生成工具，还是打造个性化的陪聊机器人，融云与 天智 的协作都能为您提供稳定、高效的解决方案。
