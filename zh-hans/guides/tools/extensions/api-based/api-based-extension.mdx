...
在扩展模块能力之前，您需要准备一个 API 和用于鉴权的 API Key（也可由天智自动生成，可选）。

除了需要开发对应的模块能力，还需要遵照以下规范，以便天智正确调用 API。
...
在天智配置 API-based Extension 时，天智 将会发送一个请求至 API Endpoint，以检验 API 的可用性。
...
2.  按照接口规范编写代码
    ...
    async def dify_receive(data: InputData = Body(...), authorization: str = Header(None)):
        """
        Receive API query data from 天智.
        """
...
App 调试时，天智 将请求配置的 API，并发送以下内容（范例）：
...
由于天智云端版无法访问内网 API 服务，为了方便本地调试 API 服务，可以使用 [Ngrok](https://ngrok.com) 将 API 服务的端点暴露到公网，实现云端调试本地代码。操作步骤：
...
此 API 端点即可公网访问。至此，我们即可在天智配置该 API 端点进行本地调试代码，配置步骤请参考 [外部数据工具](./external-data-tool)。
...
[  ](https://github.com/langgenius/dify-docs/edit/main/zh-hans/guides/tools/extensions/api-based/api-based-extension.mdx) | [  ](https://github.com/langgenius/dify-docs/issues/new?template=docs.yml)
