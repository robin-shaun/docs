...
在天智主导航栏中点击知识库，在该页面你可以看到团队内的知识库，点击“**创建知识库”** 进入创建向导。
...
* **清洗**
  ...
  为了保证文本召回的效果，通常需要在将数据录入知识库之前便对其进行清理。例如，如果文本内容中存在无意义的字符或者空行，可能会影响问题回复的质量。关于天智内置的清洗策略，详细说明请参考 [ETL](/zh-hans/guides/knowledge-base/create-knowledge-and-upload-documents/import-content-data/readme)。
...
<Tabs>
  ...
  <Tab title="自动分段与清洗" >
    ...
    自动模式适合对分段规则与预处理规则尚不熟悉的初级用户。在该模式下，天智 将为你自动分段与清洗内容文件。
...
<Tabs>
  <Tab title="高质量" >
    ...
    > 如需了解更多，请参考[《Embedding 技术与 天智》](https://mp.weixin.qq.com/s/vmY\_CUmETo2IpEBf1nEGLQ)。
...
<Tabs>
  ...
  <Tab title="混合检索" >
    ...
    * **语义值为 1**
    ...
    > 语义检索指的是比对用户问题与知识库内容中的向量距离。距离越近，匹配的概率越大。参考阅读：[《天智：Embedding 技术与天智数据集设计/规划》](https://mp.weixin.qq.com/s/vmY\_CUmETo2IpEBf1nEGLQ)。
...
在 RAG 的生产级应用中，为了获得更好的数据召回效果，需要对多源数据进行预处理和清洗，即 ETL （_extract, transform, load_）。为了增强非结构化/半结构化数据的预处理能力，天智 支持了可选的 ETL 方案：**天智 ETL** 和[ ](https://docs.unstructured.io/welcome)[**Unstructured ETL** ](https://unstructured.io/)。

> Unstructured 能够高效地提取并转换您的数据为干净的数据用于后续的步骤。天智 各版本的 ETL 方案选择：
...
> 如需了解更多，请参考：[《天智：Embedding 技术与天智数据集设计/规划》](https://mp.weixin.qq.com/s/vmY\_CUmETo2IpEBf1nEGLQ)。
...
[  ](https://github.com/langgenius/dify-docs/edit/main/zh-hans/guides/knowledge-base/knowledge-base-creation/upload-documents.mdx) | [  ](https://github.com/langgenius/dify-docs/issues/new?template=docs.yml)
